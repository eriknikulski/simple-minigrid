...
from simple_minigrid.core.constants import COLOR_TO_IDX, IDX_TO_OBJECT, OBJECT_TO_IDX
...
class GraphTransforms:
    ...
    def minigrid_to_dense_graph(
        minigrids: np.ndarray | list[SimpleMiniGridEnv],
        node_attr=None,
        edge_config=None,
    ) -> list[nx.Graph]:
        if isinstance(minigrids[0], np.ndarray):
            ...
            layouts = minigrids[..., 0]
        elif isinstance(minigrids[0], SimpleMiniGridEnv):
